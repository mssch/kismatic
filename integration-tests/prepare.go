...
EOF`
	...
	installKubeletYum         = `sudo yum -y install kubelet-1.9.1-0`
	installNFSUtilsYum        = `sudo yum -y install nfs-utils` // required for the kubelet
	installKubectlYum         = `sudo yum -y install kubectl-1.9.1-0`
	...
	installKubeletApt       = `sudo apt-get -y install kubelet=1.9.1-00`
	stopKubeletService      = `sudo systemctl stop kubelet`
	installNFSCommonApt     = `sudo apt-get -y install nfs-common`
	installKubectlApt       = `sudo apt-get -y install kubectl=1.9.1-00`
