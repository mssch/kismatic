...
EOF`
	...
	installKubeletYum         = `sudo yum -y install kubelet-1.10.4-0`
	installNFSUtilsYum        = `sudo yum -y install nfs-utils` // required for the kubelet
	installKubectlYum         = `sudo yum -y install kubectl-1.10.4-0`
	...
	installKubeletApt       = `sudo apt-get -y install kubelet=1.10.4-00`
	stopKubeletService      = `sudo systemctl stop kubelet`
	installNFSCommonApt     = `sudo apt-get -y install nfs-common`
	installKubectlApt       = `sudo apt-get -y install kubectl=1.10.4-00`
