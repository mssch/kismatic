...
func verifyRBAC(master NodeDeets, sshKey string) error {
	...
	commands := []string{
		...
		"sudo kubectl --kubeconfig /root/.kube/config create -f /tmp/pod-reader.yaml",
		...
		"sudo kubectl --kubeconfig /root/.kube/config config set-credentials jane --client-certificate=/tmp/jane.pem --client-key=/tmp/jane-key.pem",
	}
	...
	err = runViaSSH([]string{"sudo kubectl --kubeconfig /root/.kube/config get pods --user=jane"}, []NodeDeets{master}, sshKey, 30*time.Second)
	if err != nil {
	...
	out, _ := executeCmd("sudo kubectl --kubeconfig /root/.kube/config get nodes --user=jane", master.PublicIP, master.SSHUser, sshKey)
